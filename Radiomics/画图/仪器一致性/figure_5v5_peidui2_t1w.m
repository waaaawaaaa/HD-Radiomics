...
% 参数设置
seq = 'T2W_FLAIR'; % T2W  T1W  T2W_FLAIR
seq_name = 'FLAIR';
excel = '腐蚀1_T2_cx';
file_root = ['H:\重建数据\mapping_unet_best\peizhun\Register_jiegouxiang_1vs1\' seq '\ROI_processed\'];
filename = [file_root seq '_' excel '_mask_zhuzhu4.xlsx'];  % 替换为你的Excel文件名
...
% 获取所有 sheet 名称
roi_values1 = {'Cd', 'Put', 'GPe', 'GPi', 'TH', 'RN', 'SN', 'DN'};
...
% 添加图例，并隐藏图例的边框
lgd = legend({'Ingenia CX', 'Ingenia Elition X'}, 'Location', 'northeast');
lgd.Box = 'off'; % 隐藏图例的边框

% title(['Bar Plot for ', seq_name]);
ylabel(['Mean ', seq_name, ' value (%)']);
...
xlabel(['Mean ', seq_name, ' value (%)']);
ylabel(['Difference in ', seq_name, ' value (%)']);
hold off;
