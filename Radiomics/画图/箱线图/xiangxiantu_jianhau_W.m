% 对我特征的统计结果画图  需要简化一下我的图，只画出有显著性差异的图
% 2025/06/08 zhumengying

clc; clear;close all;

% 读取数据
seq = 't1w_t2w';  % T2Mapping 或 T1W T2W T2W_FLAIR t1w_t2w
excel = '腐蚀1_T2_cx_mask';
% file_root = 'H:/重建数据/mapping_unet_best/peizhun/PD_out_t2m/ROI_processed/';
file_root = ['H:/重建数据/mapping_unet_best/peizhun/Register_jiegouxiang_1vs1/' seq '/ROI_processed/'];
% filename = fullfile(file_root, [seq '_' excel '0.xlsx']);
filename = fullfile(file_root, ['test3_' seq '_' excel '_zhuzhu4.xlsx']);

% 获取所有 sheet 名称
roi_values = {'Cd', 'Put', 'GPe', 'GPi', 'TH', 'RN', 'SN', 'DN'};
roi_names = {'Cd', 'Put', 'GPe', 'GPi', 'TH', 'RN', 'SN', 'DN'};
    ...
    for i = 1:length(roi_values)
        ...
        if roi_test.p_test>0.05  % 没有显著性
           continue
            ...
            mean_tezheng = [mean_tezheng, roi_test.mean_hc,roi_test.mean_pre_hd,roi_test.mean_hd];   % 提取出三组的均值特征
...
%     figure('Position',[100, 100, 35*length(group_positions)+round(1/length(group_positions)*40), 450]);
    fig=figure('Units', 'pixels','Position', [100, 100, 20*length(group_positions)+80, 500]);
        % 创建一个 axes 并精确控制其绘图区域
    ax = axes('Units', 'pixels', ...
              'Position', [70, 50, 20*length(group_positions), 450], ...
              'TickDir', 'in', ...
              'Box', 'on');
    hold(ax, 'on');
    
    % 绘制箱线图
    boxplot(ax,all_data_combined, 'colors', 'k', 'symbol', '', 'Widths',0.4, 'positions',group_positions);
    y_max = find_max_box(all_data_combined);
    y_min = find_min_box(all_data_combined);
    range = y_max - y_min;
    ylim(ax,[y_min-range*0.02, y_max+range*0.09]);  % 设置主 Y 轴范围为 0~0.1
    
    scatter(ax, group_positions, mean_tezheng, '*', 'Color', filledcolor1)
    ...
    xticks(ax,group_positions(3*(1:length(group_positions) / 3) - 1));
    xticklabels(ax,roi_names_p);

    % 添加每组之间的垂直分隔虚线
    n_rois = length(roi_values);
    for i = 1:length(group_positions) / 3
        % 标出显著性
...
%         y_limits = 1.05*ylim;
%         aaa=ylim;
%         y_limits = [aaa(1),1.03*aaa(2)];
        y_limits = ylim;
    
        % 绘制虚线
        plot(ax,[line_x, line_x], y_limits, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1);
    end
...
%     title(['特征: ', feature], 'Interpreter', 'none')
    ...
    set(ax, 'FontSize', 10)
    box(ax, 'off');   
    % 让 figure 自动适应内容（关键一步）
    set(gca,'Position', [70, 50, 20*length(group_positions), 450])
%     drawnow;
%     set(fig, 'PaperPositionMode', 'auto');  % 自动扩展 figure 大小以容纳标签等
end
...
%         disp('无显著性差异');
    ...
    % 定义组间比较的p值和对应组名
    comparisons = {
        'hc', 'pre_hd', extractCellValue(all_test_roi.hc_vs_pre_hd);  % 注意：使用 {} 访问 cell 内容
        'hc', 'hd', extractCellValue(all_test_roi.hc_vs_hd);
        'pre_hd', 'hd', extractCellValue(all_test_roi.pre_hd_vs_hd)
    };
