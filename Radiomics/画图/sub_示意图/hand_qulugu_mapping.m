...
seq = 'T2star_Mapping';% T2star_Mapping

file_root = ['H:\重建数据\mapping_unet_best\swapdim\' seq '2\' sub];
filename = [sub '_' seq '_swapdim.nii.gz'];
out_root = ['H:\重建数据\mapping_unet_best\swapdim\' seq '2'];
% 拼接输出文件名
...
for i = 1:length(selected_slices)
    ...
    % 显示当前切片，根据需要调整显示范围
    figure;
    imshow(slice_data, [0, 2]); colormap jet;% 根据实际图像灰度范围调整 [0, 6000]，这里需要根据实际情况调整
...
figure();imshow3D(nii_data_mask,[0,2]),colormap jet;
