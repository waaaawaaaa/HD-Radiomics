%配准将标准空间的PD模版配准到T2star_Mapping  使用ANTS
% 现在将其他2个模态配准到T2M，再使用PD的T1W R2共同配准到受试者的T1W T2W

% 最有应用再LABEL中

%需要再终端输入MATLAB打开，从桌面进去的画跑不了 在终端输入：
% export LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libstdc++.so.6  # 确保这个路径是系统的libstdc++.so.6路径
% matlab
 
% 2025/01/12  zhumengying
% final

clc; clear;

% Set up paths and parameters
root = '/data1/zmy3/HD/HD_data/';
file_root = [root 'qulugu'];
file_t1t2_root = [root 'normalize_99'];
out_root = [root 'PD_out_t2s']; % Save path
seq_t2m = 'T2Mapping';
seq_t2s = 'T2star_Mapping';
seq_t1w = 'T1W';
seq_t2w = 'T2W';
seq_t2flair = 'T2W_FLAIR';
ants_path = '/opt/ants/bin/'; % Path to ANTs binaries

root_pd = '/data1/zmy3/HD_registration/zmy2/';
PD_T1 = [root_pd 'HybraPD/PD_template_T1_flip_pa.nii.gz'];
% /data1/zmy3/HD_registration/zmy2/HybraPD/PD_template_T1_flip_pa.nii
PD_R2 = [root_pd 'HybraPD/PD_template_R2_flip_pa.nii.gz'];
PD_LABEL = [root_pd 'HybraPD/PD_template_label_Whole_flip_pa.nii.gz'];

% ANTs commands
ants_registration = [ants_path 'antsRegistrationSyN.sh'];
ants_apply_transforms = [ants_path 'antsApplyTransforms'];
tmp2ind_path = '/data1/zmy3/HD_registration/qulugu/tmp2ind_T1T2_zmy.sh';

% Create output directories if they do not exist
outpath_pd = fullfile(out_root, 'zmy_PD2T2W');
if ~exist(outpath_pd, 'dir')
    mkdir(outpath_pd);
end


outpath_t1w = fullfile(out_root, seq_t1w);
if ~exist(outpath_t1w, 'dir')
    mkdir(outpath_t1w);
end

outpath_t2w = fullfile(out_root, seq_t2w);
if ~exist(outpath_t2w, 'dir')
    mkdir(outpath_t2w);
end

outpath_t2flair = fullfile(out_root, seq_t2flair);
if ~exist(outpath_t2flair, 'dir')
    mkdir(outpath_t2flair);
end

% List files in the directory
filePattern = fullfile(file_root, seq_t2s, ['*' seq_t2s '_brain.nii.gz']);
files = dir(filePattern);

%%
% 处理每个文件
for i = 1:length(files)
    T2star_Map_File = fullfile(files(i).folder, files(i).name);
    filename = strrep(files(i).name, ['_' seq_t2s '_brain.nii.gz'], '');  %sub001_HC  sub001

    T2Map_File = fullfile(file_root, seq_t2m, [filename '_' seq_t2m '_brain.nii.gz']);
    T2W_File = fullfile(file_t1t2_root, seq_t2w, [filename '_' seq_t2w '_normalized.nii.gz']);
    T1W_File = fullfile(file_t1t2_root, seq_t1w, [filename '_' seq_t1w '_normalized.nii.gz']);
    T2W_flair_File = fullfile(file_t1t2_root, seq_t2flair, [filename '_' seq_t2flair '_normalized.nii.gz']);

    % STEP 1: Register T1W to T2star mapping
    outputFile_t1w = fullfile(outpath_t1w, [filename '_' seq_t1w '_']);
    if ~exist([outputFile_t1w '1Warp.nii.gz'], 'file') || ~exist([outputFile_t1w '0GenericAffine.mat'], 'file')
        cmd1 = [ants_registration ' -d 3 -f ' T2star_Map_File ' -m ' T1W_File ' -o ' outputFile_t1w ' -n 8'];
        disp(['Running: ' cmd1]);
        system(cmd1);
    else
        disp(['Skipping T1W to T2star mapping registration for ' filename ' (results already exist)']);
    end

    % STEP 2: Register T2W mapping to T2star mapping
    outputFile_t2w = fullfile(outpath_t2w, [filename '_' seq_t2w '_']);
    if ~exist([outputFile_t2w '1Warp.nii.gz'], 'file') || ~exist([outputFile_t2w '0GenericAffine.mat'], 'file')
        cmd2 = [ants_registration ' -d 3 -f ' T2star_Map_File ' -m ' T2W_File ' -o ' outputFile_t2w ' -n 8'];
        disp(['Running: ' cmd2]);
        system(cmd2);
    else
        disp(['Skipping T2W to T2star mapping registration for ' filename ' (results already exist)']);
    end

    % STEP 3: Register T2flair mapping to T2star mapping
    outputFile_t2flair = fullfile(outpath_t2flair, [filename '_' seq_t2flair '_']);
    if ~exist([outputFile_t2flair '1Warp.nii.gz'], 'file') || ~exist([outputFile_t2flair '0GenericAffine.mat'], 'file')
        cmd3 = [ants_registration ' -d 3 -f ' T2star_Map_File ' -m ' T2W_flair_File ' -o ' outputFile_t2flair ' -n 8'];
        disp(['Running: ' cmd3]);
        system(cmd3);
    else
        disp(['Skipping T2 flair to T2star mapping registration for ' filename ' (results already exist)']);
    end
    

    % STEP 1: Register PD to T2W t1w 
    out_t1t2 = fullfile(outpath_pd, [filename '_t1wt2w_t1r2']);
    t2w_warped = [outputFile_t2w 'Warped.nii.gz'];
    t1w_warped = [outputFile_t1w 'Warped.nii.gz'];
    t2flair_warped = [outputFile_t2flair 'Warped.nii.gz'];

    cmd_t1t2 = [tmp2ind_path ' --T1ind=' t1w_warped ' --T2ind=' t2w_warped ' --T1tmp=' PD_T1 ...
        ' --T2tmp=' PD_R2 ' --Atmp=' PD_LABEL ' --outdir=' out_t1t2 ' --nthreads=16'];

    disp(['Running: ' cmd_t1t2]);
    system(cmd_t1t2);
end


