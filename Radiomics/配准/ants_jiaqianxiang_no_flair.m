...
% 先将T2W配准到T1W，再使用PD的T1W R2共同配准到受试者的T1W T2W
...
out_root = [root 'Register_jiegouxiang_no_flair']; % Save path
seq_t1w = 'T1W';
seq_t2w = 'T2W';
...
outpath_pd = fullfile(out_root, 'PD2T1W');
if ~exist(outpath_pd, 'dir')
    mkdir(outpath_pd);
end


outpath_t1w = fullfile(out_root, [seq_t1w '_2_t1w']);
if ~exist(outpath_t1w, 'dir')
    mkdir(outpath_t1w);
end

outpath_t2w = fullfile(out_root, [seq_t2w '_2_t1w']);
...
%     end
    ...
    if ~exist([outputFile_t2w '1Warp.nii.gz'], 'file') || ~exist([outputFile_t2w '0GenericAffine.mat'], 'file')
        cmd1 = [ants_registration ' -d 3 -f ' T1W_File ' -m ' T2W_File ' -o ' outputFile_t2w ' -n 8'];
        disp(['Running: ' cmd1]);
        system(cmd1);
    else
        disp(['Skipping T2W to T1W registration for ' filename ' (results already exist)']);
    end
    ...
    cmd_t1t2 = [tmp2ind_path ' --T1ind=' T1W_File ' --T2ind=' t2w_warped ' --T1tmp=' PD_T1 ...
