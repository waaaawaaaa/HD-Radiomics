%配准将标准空间的PD模版配准到T2 Mapping  使用ANTS
...
out_root = [root 'PD_out_t2m']; % Save path
seq_t2m = 'T2Mapping';
...
for i = 1:length(files)
    ...
    % STEP 1: Register T1W to T2 mapping
    ...
    if ~exist([outputFile_t1w '1Warp.nii.gz'], 'file') || ~exist([outputFile_t1w '0GenericAffine.mat'], 'file')
        cmd1 = [ants_registration ' -d 3 -f ' T2Map_File ' -m ' T1W_File ' -o ' outputFile_t1w ' -n 8'];
        disp(['Running: ' cmd1]);
        system(cmd1);
    else
    ...
    % STEP 2: Register T2W mapping to T2 mapping
    ...
    if ~exist([outputFile_t2w '1Warp.nii.gz'], 'file') || ~exist([outputFile_t2w '0GenericAffine.mat'], 'file')
        cmd2 = [ants_registration ' -d 3 -f ' T2Map_File ' -m ' T2W_File ' -o ' outputFile_t2w ' -n 8'];
        disp(['Running: ' cmd2]);
        system(cmd2);
    else
    ...
    % STEP 3: Register T2flair mapping to T2 mapping
    ...
    if ~exist([outputFile_t2flair '1Warp.nii.gz'], 'file') || ~exist([outputFile_t2flair '0GenericAffine.mat'], 'file')
        cmd3 = [ants_registration ' -d 3 -f ' T2Map_File ' -m ' T2W_flair_File ' -o ' outputFile_t2flair ' -n 8'];
        disp(['Running: ' cmd3]);
        system(cmd3);
    else
