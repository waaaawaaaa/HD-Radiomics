...
% 设置输入文件路径
seq = 'T1W';
input_file_root = ['\Register_jiegouxiang_1vs1\' seq];
input_file_seq = ['\normalize_99\' seq];  %去颅骨后T2mapping的路径，只是作为叠在底下的文件
...
filePattern = fullfile(input_file_root, 'sub*_PD_LABEL_registered.nii.gz'); % 35个受试者
...
%         修正 mask，将 seq_image 中对应位置值大于2的部分也加入 mask
        ...
        if sum(pro_roiMask(:))<5
            ...
            % 将字符串写入到文件
            fprintf(fileID, '%s\n', outputStr);
            pro_roiMask_raw(seq_image < 0.7) = 0;  % 给他多设置一个阈值
