...
seq = 'T2W_FLAIR' # T1W  T2W  T2W_FLAIR
excel = '腐蚀1_T2_cx_mask_zhuzhu4' # 腐蚀1_T2_cx  腐蚀1_T2_cx_mask
# 基础路径
base_path = r'H:\重建数据\mapping_unet_best\peizhun\Register_jiegouxiang_1vs1'
file_root = os.path.join(base_path, seq, 'ROI_processed')
filename = os.path.join(file_root, f'{seq}_{excel}.xlsx')
data = pd.read_excel(filename)

# 输出文件
output_filename = f'test3_{seq}_{excel}.xlsx'
...
for roi in roi_values:
    ...
    features_name = sorted_table.columns[:-2]
    ...
    comparisons_data1 = result_table.iloc[:, np.r_[0:6, 12:15]]
    all_comparisons1.append(comparisons_data1)
    ...
    with pd.ExcelWriter(excel_output, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
        ...
        styled_result_table.to_excel(writer, sheet_name=str(roi), startrow=0, startcol=data_table.shape[1] + 1,
                                     index=False)  # 从第 64 列开始写入
...
all_comparisons_df1 = pd.concat(all_comparisons1, axis=0).reset_index(drop=True)
feature_series = pd.Series(np.tile(roi_column[:-1], 8), name='feature')
roi_series = pd.Series(np.repeat(roi_values, 13), name='ROI')
all_comparisons_df1.insert(0, 'feature', feature_series)
all_comparisons_df1.insert(0, 'ROI', roi_series)
# 再次尝试四舍五入
# all_comparisons_df1 = all_comparisons_df1.round(4)
...
# 保存到 Excel
with pd.ExcelWriter(excel_output, engine='openpyxl', mode='a', if_sheet_exists='new') as writer:
    all_comparisons_df1.to_excel(writer, sheet_name='All_ROI_Comparisons1', startrow=1, startcol=0, index=False)
print(f'统计结果和数据表已成功保存到 {excel_output} 的 All_ROI_Comparisons1 表单中！')
