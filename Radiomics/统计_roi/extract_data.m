...
outputExcelFile = [input_file_root '_腐蚀1_T2_cx.xlsx'];

% 初始化保存统计特征的变量
stats = struct('ROI', {}, 'Mean', {}, 'Variance', {}, 'StdDev', {}, 'Median', {}, 'RMS', {}, 'Skewness', {}, 'Kurtosis', {}, ...
               'Percentile_10th', {}, 'Percentile_25th', {}, 'Percentile_50th', {}, 'Percentile_75th', {}, 'Percentile_90th', {}, ...
               'Min', {}, 'Max', {}, 'numVoxels', {}, 'volumes', {}, 'age', {}, 'CAG', {}, 'CAP', {}, 'ClassicValue', {});

% 读取亨廷顿_表型数据.xlsx 文件
phenotypeData = readtable(phenotypeFile, 'Range', 'A1:S70', 'VariableNamingRule', 'preserve');

% 列出文件夹中以序列名+.nii.gz结尾的文件
filePattern = fullfile(input_file_root,'T2_腐蚀1', ['sub*' '.nii.gz']);%35个受试者
...
for i = 1:length(files)
    ...
    for j = 1:length(roiInfo)
        ...
        if ~isempty(Data)
            ...
            stats(end).Percentile_10th = percentile_values(1);
            stats(end).Percentile_25th = percentile_values(2);
            stats(end).Percentile_50th = percentile_values(3);
            stats(end).Percentile_75th = percentile_values(4);
            stats(end).Percentile_90th = percentile_values(5);
            
            stats(end).Min = min(Data); % 最小值
            stats(end).Max = max(Data); % 最大值
            stats(end).numVoxels = length(Data); % 占多少个像素
            stats(end).volumes = caculate_volume(length(Data)); % 体积计算
            stats(end).age = age;
            stats(end).CAG = CAG;
