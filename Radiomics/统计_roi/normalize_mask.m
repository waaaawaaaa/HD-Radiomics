...
clc;clear;
file_root = 'H:\重建数据\zmy_final\';
seqs = {'T1W', 'T2W', 'T2W_FLAIR'};
% file_output = fullfile(file_root,'normalize_99');

for s = 1:length(seqs)
    file_seq = fullfile(file_root,'qulugu', seqs{s});
    files = dir(fullfile(file_seq, '*brain.nii.gz'));

    seq_output = fullfile('H:\重建数据\mapping_unet_best','normalize_99_mask', seqs{s});

    % 创建输出文件夹，如果不存在的话
    if ~exist(seq_output, 'dir')
        mkdir(seq_output);
    end
    % 遍历找到的文件，并进行处理
...
%         nii_data_max = max(nii_data(:));  % 获取图像的最大值
        ...
        % 构建输出文件名
        output_filename = fullfile(seq_output, [base_name '_' seqs{s} '_normalized.nii']);
